...
from profiles.models import Availability, InterestChoice, UserProfile
...
class MatchingService:
    ...
    def find_potential_matches(self, limit=20):
        ...
        queryset = UserProfile.objects.exclude(
            ...
            is_profile_complete=True, user__is_active=True, user__is_suspended=False
        )

        # Filter by gender preferences if specified
        if self.match_preferences.preferred_genders:
            queryset = queryset.filter(
                gender__in=self.match_preferences.preferred_genders
            )

        # Filter by age range if birthdate is provided
        if self.user_profile.birth_date:
            min_birth_date = datetime.now().date() - timedelta(
                days=365 * self.match_preferences.max_age
            )
            max_birth_date = datetime.now().date() - timedelta(
                days=365 * self.match_preferences.min_age
            )
            queryset = queryset.filter(
                birth_date__gte=min_birth_date, birth_date__lte=max_birth_date
            )
    ...
    def create_match(self, responder_id):
        ...
        # Calculate scores
        # Interest similarity
        initiator_interests = set(
            initiator_profile.interests.values_list("id", flat=True)
        )
        responder_interests = set(
            responder_profile.interests.values_list("id", flat=True)
        )
        ...
        distance_score = 0
        if (
            initiator_profile.latitude
            and initiator_profile.longitude
            and responder_profile.latitude
            and responder_profile.longitude
        ):
            distance = self._calculate_distance(
                initiator_profile.latitude,
                initiator_profile.longitude,
                responder_profile.latitude,
                responder_profile.longitude,
            )
    ...
    def _calculate_availability_score(self, user_availabilities, match_availabilities):
        ...
        availability_score = min(
            100, (total_overlap_minutes / max_overlap_minutes) * 100
        )
